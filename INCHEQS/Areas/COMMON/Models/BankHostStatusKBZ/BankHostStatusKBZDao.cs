...
namespace INCHEQS.Areas.COMMON.Models.BankHostStatusKBZ
{
    public class BankHostStatusKBZDao : IBankHostStatusKBZDao
    {
        ...
        public string GetHostReturnReasonDesc(string fldBankHostStatusCode)
        {
            ...
            string result = "";
            string stmt = "SELECT * FROM tblBankHostStatusMaster where fldStatusID=@fldBankHostStatusCode";
        ...
        public BankHostStatusKBZModel GetBankHostStatusMasterKBZ(string fldBankHostStatusCode)
        {
            ...
            resultTable = dbContext.GetRecordsAsDataTableSP("spcgBankHostStatusMasterMAB", sqlParameterNext.ToArray());
            if (resultTable.Rows.Count > 0)
            {
        ...
        public BankHostStatusKBZModel GetBankHostStatusMasterKBZTemp(string fldBankHostStatusCode)
        {
            ...
            BankHostStatusKBZModel bankHostStatus = new BankHostStatusKBZModel();
            sqlParameterNext.Add(new SqlParameter("@fldBankHostStatusCode", fldBankHostStatusCode));
            resultTable = dbContext.GetRecordsAsDataTableSP("spcgBankHostStatusMasterMABTemp", sqlParameterNext.ToArray());
            if (resultTable.Rows.Count > 0)
            {
                DataRow row = resultTable.Rows[0];
                bankHostStatus.fldBankHostStatusCode = row["fldBankHostStatusCode"].ToString();
        ...
        //public BankHostReturnStatusKBZModel GetMABBankHostReturnStatus(string itemId)
        //{
        //    DataTable resultTable = new DataTable();
        //    List<SqlParameter> sqlParameterNext = new List<SqlParameter>();
        //    BankHostReturnStatusKBZModel bankHostReturnStatus = new List<BankHostReturnStatusKBZModel>();
        //    sqlParameterNext.Add(new SqlParameter("@fldItemId", itemId));
        //    resultTable = dbContext.GetRecordsAsDataTableSP("spcgMABBankHostReturnStatus", sqlParameterNext.ToArray());
        //    if (resultTable.Rows.Count > 0)
        //    {

        //        foreach (DataRow row in resultTable.Rows)
        //        {
        //            bankHostReturnStatus.itemId = row["fldItemID"].ToString();
        //            bankHostReturnStatus.statusCode = row["fldPostingReturnCode"].ToString();
        //            bankHostReturnStatus.statusDesc = row["fldPostingReturnDesc"].ToString();

        //            bankHostReturnStatus.();
        //        }

        //    }

        //    return bankHostReturnStatus;
        //}


        public List<BankHostReturnStatusKBZModel> GetMABBankHostReturnStatus(string itemId)
        {
            List<BankHostReturnStatusKBZModel> listHostReturnStatus = new List<BankHostReturnStatusKBZModel>();
            ...
            try
            {
                ...
                dataTable = dbContext.GetRecordsAsDataTableSP("spcgMABBankHostReturnStatus", sqlParameterNext.ToArray());
                if (dataTable.Rows.Count > 0)
                {
                    foreach (DataRow row in dataTable.Rows)
                    {
                        BankHostReturnStatusKBZModel HostReturnStatus = new BankHostReturnStatusKBZModel()
                        {
                            itemId = row["fldItemId"].ToString(),
                            statusCode = row["fldPostingReturnCode"].ToString(),
                            statusDesc = row["fldPostingReturnDesc"].ToString()

                        };
                        listHostReturnStatus.Add(HostReturnStatus);
                    }
                }
            }
        ...
        public bool UpdateHostStatusMasterKBZ(FormCollection col, AccountModel currentUser)
        {
            ...
            intRowAffected = dbContext.ExecuteNonQuery(CommandType.StoredProcedure, "spcuBankHostStatusMasterMAB", sqlParameterNext.ToArray());
            if (intRowAffected > 0)
            {
                blnResult = true;
            }
        ...
        public bool AddBankHostCodeinKBZTemptoUpdate(FormCollection col, AccountModel currentUser)
        {
            ...
            intRowAffected = dbContext.ExecuteNonQuery(CommandType.StoredProcedure, "spciBankHostStatusMasterMABUpdateTemp", sqlParameterNext.ToArray());
            if (intRowAffected > 0)
            {
                blnResult = true;
            }
        ...
        public bool CreateBankHostStatusMasterKBZTemp(FormCollection col)
        {
            ...
            intRowAffected = dbContext.ExecuteNonQuery(CommandType.StoredProcedure, "spciBankHostStatusMasterMABTemp", sqlParameterNext.ToArray());
        ...
        public bool DeleteInBankHostStatusMasterKBZ(string fldBankHostStatusCode)
        {
            ...
            List<SqlParameter> sqlParameterNext = new List<SqlParameter>();
            sqlParameterNext.Add(new SqlParameter("@fldBankHostStatusCode", fldBankHostStatusCode));
            intRowAffected = dbContext.ExecuteNonQuery(CommandType.StoredProcedure, "spcdBankHostStatusMasterMAB", sqlParameterNext.ToArray());
            if (intRowAffected > 0)
            {
                blnResult = true;
            }
        ...
        public bool CreateInBankHostStatusMasterKBZ(FormCollection col)
        {
            ...
            intRowAffected = dbContext.ExecuteNonQuery(CommandType.StoredProcedure, "spciBankHostStatusMasterMAB", sqlParameterNext.ToArray());
        ...
        public bool AddtoBankHostStatusMasterKBZTempToDelete(string fldBankHostStatusCode)
        {
            ...
            intRowAffected = dbContext.ExecuteNonQuery(CommandType.StoredProcedure, "spciBankHostCodeMABinTemptoDelete", sqlParameterNext.ToArray());
        ...
        public bool CheckBankHostCodeDataTempKBZ(string fldBankHostStatusCode)
        {
            ...
            List<SqlParameter> sqlParameterNext = new List<SqlParameter>();
            sqlParameterNext.Add(new SqlParameter("@fldBankHostStatusCode", fldBankHostStatusCode));
            resultTable = dbContext.GetRecordsAsDataTableSP("spcgBankHostCodeTempDataMAB", sqlParameterNext.ToArray());
        ...
        public bool CreateBankHostStatusCodeinMainKBZ (string fldBankHostStatusCode)
        {
            ...
            List<SqlParameter> sqlParameterNext = new List<SqlParameter>();
            sqlParameterNext.Add(new SqlParameter("@fldBankHostStatusCode", fldBankHostStatusCode));
            intRowAffected = dbContext.ExecuteNonQuery(CommandType.StoredProcedure, "spciBankHostStatusCodeMABinMain", sqlParameterNext.ToArray());
            if (intRowAffected > 0)
            {
                blnResult = true;
            }
        ...
        public BankHostStatusKBZModel GetBankHostStatusCodeData(string fldBankHostStatusCode)
        {
            ...
            List<SqlParameter> sqlParameterNext = new List<SqlParameter>();
            sqlParameterNext.Add(new SqlParameter("@fldBankHostStatusCode", @fldBankHostStatusCode));
            resultTable = dbContext.GetRecordsAsDataTableSP("spcgBankHostCodeMABDatabyId", sqlParameterNext.ToArray());
        ...
        public bool DeleteInBankHostStatusCodeKBZ(string fldBankHostStatusCode)
        {
            ...
            List<SqlParameter> sqlParameterNext = new List<SqlParameter>();
            sqlParameterNext.Add(new SqlParameter("@fldBankHostStatusCode", fldBankHostStatusCode));
            intRowAffected = dbContext.ExecuteNonQuery(CommandType.StoredProcedure, "spcdBankHostStatusCodeMAB", sqlParameterNext.ToArray());
            if (intRowAffected > 0)
            {
                blnResult = true;
            }
        ...
        public bool UpdateBankHostStatusCodeToMainById(string fldBankHostStatusCode)
        {
            ...
            List<SqlParameter> sqlParameterNext = new List<SqlParameter>();
            sqlParameterNext.Add(new SqlParameter("@fldBankHostStatusCode", fldBankHostStatusCode));
            resultTable = dbContext.GetRecordsAsDataTableSP("spcuBankHostStatusCodeMABToMainById", sqlParameterNext.ToArray());
        ...
        public bool DeleteBankHostStatusCodeinTempKBZ(string fldBankHostStatusCode)
        {
            ...
            List<SqlParameter> sqlParameterNext = new List<SqlParameter>();
            sqlParameterNext.Add(new SqlParameter("@fldBankHostStatusCode", fldBankHostStatusCode));
            intRowAffected = dbContext.ExecuteNonQuery(CommandType.StoredProcedure, "spcdBankHostStatusCodeMABinTemp", sqlParameterNext.ToArray());
            if (intRowAffected > 0)
            {
                blnResult = true;
            }
