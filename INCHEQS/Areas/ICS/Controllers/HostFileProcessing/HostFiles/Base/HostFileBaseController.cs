...
namespace INCHEQS.Areas.ICS.Controllers.InwardClearing.HostFiles
{
    public abstract class HostFileBaseController : BaseController {
        ...
        public virtual async Task<ActionResult> SearchResultPage(FormCollection collection, string type) {
            initializeBeforeAction();
            if (pageSqlConfig.TaskId == "3110")
            {
                if (type == "daily")
                {
                    ViewBag.SearchResult = await pageConfigDao.getResultListFromDatabaseViewAsync(new PageSqlConfig(pageSqlConfig.TaskId, "View_LoadDailyFile", null, "DATEDIFF(D,fldClearDate,@fldClearDate) = 0", new[] {
                    new SqlParameter("@fldClearDate",DateTime.Today.ToString("yyyy-MM-dd"))}), collection);
                }
                else
                {
