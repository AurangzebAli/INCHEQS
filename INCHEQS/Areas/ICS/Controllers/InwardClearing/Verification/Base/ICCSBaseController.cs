...
,
            ...
            public async Task<ActionResult> DeferedToPhysical(FormCollection collection)
            {
                await initializeBeforeAction();
                string inwardItemId = collection["fldInwardItemId"];
                /*Boolean checkResult = true;
                ...
                Log(DateTime.Now + ":Get inward item id :" + inwardItemId, CurrentUser.Account.BankCode);
                Log(DateTime.Now + ":Get inward item UIC :" + collection["current_fldUIC"], CurrentUser.Account.BankCode);
                Log(DateTime.Now + ":Get inward item check no :" + collection["current_fldChequeSerialNo"], CurrentUser.Account.BankCode);
                Log(DateTime.Now + ":Get inward item acc no :" + collection["current_fldAccountNumber"], CurrentUser.Account.BankCode);
                Log(DateTime.Now + ":Get inward item image name :" + collection["imageId"], CurrentUser.Account.BankCode);
                string verifyAction = VerificationStatus.ACTION.Manual;
                Log(DateTime.Now + ":Start validate verification ", CurrentUser.Account.BankCode);
                List<string> errorMessages = verificationDao.ValidateVerification(collection, CurrentUser.Account, verifyAction, staskid);
                ...
                else
                {
                    ...
                    //check next available cheque
                    Log(DateTime.Now + ":Check available check ", CurrentUser.Account.BankCode);
                    if (staskid == "306310"  || staskid == "306910" || staskid == "306920" || staskid == "306930" || staskid == "308140" || staskid == "308130" || staskid == "306550" || staskid == "306210" || staskid == "306240" || staskid == "306220" || staskid == "306230" || staskid == "306530" || staskid == "306540" || staskid == "306510" || staskid == "306520" || staskid == "309100")
                    {

                        ViewBag.DeferToPhysical = commonInwardItemDao.DeferedToPhysicalUpdated(collection["fldInwardItemId"], CurrentUser.Account, collection, collection["fldAccountNumber"], collection["fldChequeSerialNo"], staskid);
