...
namespace INCHEQS.Areas.OCS.Controllers
{
    ...
    namespace OutwardClearing
    {
        ...
        //[CustomAuthorize]
        public abstract class ChequeAccountEntryBaseController : BaseController
        {
            ...
            public ChequeAccountEntryBaseController(IPageConfigDaoOCS pageConfigDao,
            ...
            protected async Task<QueueSqlConfig> initializeBeforeAction(string taskId = null)
            {
                ...
                else
                {
                    ...
                    gQueueSqlConfigSearch = await pageConfigDao.GetQueueConfigAsync(Convert.ToString(Convert.ToInt64(staskid) + 1), CurrentUser.Account);
                }
                ...
                {
                    systemProfileDao.Log("ChequeAccountEntryBaseController/initializeBeforeAction error :" + ex.Message, CurrentUser.Account.Logindicator, CurrentUser.Account.LogPath, CurrentUser.Account.UserId, CurrentUser.Account.UserAbbr);
            ...
            {
                await initializeBeforeAction();
                string scannerbatch = collection["fldbatchnumber"];
                string flduserid = CurrentUser.Account.UserId;
                string fldtype = "1";
                string userbankcode = CurrentUser.Account.BankCode;
                string fldcapturingdate = collection["fldcapturingdate"];
                string fldbankcode = collection["fldbankcode"];
                string fldbranchcode = collection["fldbranchcode"];
                string fldscannerid = collection["fldscannerid"];
                string flduic = collection["flduic"];
                //await pageConfigDao.updateQueueSessionAsync(scannerbatch, flduserid, flduic, fldtype, userbankcode, fldscannerid, fldcapturingdate, fldbankcode, fldbranchcode);
                //string sqlCondition = "";
                int checkitem = 1;
            ...
            public virtual async Task<ActionResult> ChequeAccountQueue(FormCollection collection)
            {
            ...
            public virtual async Task<ActionResult> PrevCheque(FormCollection collection)
            {
                ...
                else
                {
                    ...
                    //OutwardItemViewModel resultModel = OutwardItemConcern.PrevChequePopulateViewModel(gQueueSqlConfig, result, collection);
                    //ViewBag.OutwardItemViewModel = resultModel;
                    //ViewBag.IQA = resultModel.getField("fldIQA").Trim();
                    //ViewBag.IQADesc = resultModel.getField("flddesc").Trim();
                    //ViewBag.MICRCorrection = resultModel.getField("fldMicrCorrectionInd").Trim();
            ...
            public virtual async Task<ActionResult> Close(FormCollection collection)
            {
                ...
                // add by shamil 20170616
                // Delete temp gif when see next check
                if (!String.IsNullOrEmpty(collection["flduic2"].Trim()))
                {
                    commonOutwardItemDao.DeleteTempGif(collection["flduic2"].Trim());
                }
