...
namespace INCHEQS.Areas.OCS.Controllers.Utilities.Search.Base
{
    ...
    namespace OutwardClearing
    {
        ...
        public abstract class SearchBaseController : BaseController
        {
            ...
            protected readonly ISearchDao searchDao;
            ...
            public SearchBaseController(IPageConfigDaoOCS pageConfigDao,
            ...
            protected async Task<QueueSqlConfig> initializeBeforeAction(string taskId = null)
            {
                ...
                try
                {
                    ...
                    if (!"Index".Equals(currentAction)
                        ...
                        & !"ChequeSearchUpdate".Equals(currentAction) 
                        & !"ChequeSearchReject".Equals(currentAction) 
                ...
                {
                    systemProfileDao.Log("SearchBaseController/initializeBeforeAction error :" + ex.Message, CurrentUser.Account.Logindicator, CurrentUser.Account.LogPath, CurrentUser.Account.UserId, CurrentUser.Account.UserAbbr);
            ...
            public virtual async Task<ActionResult> Index()
            {
                ...
                //ViewBag.SearchPage = pageConfigDao.GetSearchFormModelFromConfig(CurrentUser.Account, new PageSqlConfig("999950","view_search"));
                auditTrailDao.SecurityLog("Search Cheque", "", staskid, CurrentUser.Account);
            ...
            public virtual async Task<ActionResult> ChequeVerificationPage(FormCollection collection)
            {
                ...
                List<SearchHistoryModel> history = await commonOutwardItemDao.GetSearchHistoryAsync(OutwardItemId);

                ViewBag.SearchHistoryModel = history;
                ViewBag.OutwardItemViewModel = resultModel;

                //credit account number
                string tmpCred = ViewBag.OutwardItemViewModel.getField("fldcreditaccno").ToString();
                string tmpCredname = ViewBag.OutwardItemViewModel.getField("fldcreditaccname").ToString();
                string tmppoststatus = ViewBag.OutwardItemViewModel.getField("fldcreditpostingstatus").ToString();
                string tmpchequeamount = ViewBag.OutwardItemViewModel.getField("fldchequeamount").ToString();

                string[] tmp = tmpCred.Split('|');
                string[] tmpname = tmpCredname.Split('|');
                string[] tmpstatus = tmppoststatus.Split('|');
                string[] tmpamount = tmpchequeamount.Split('|');

                List<OutwardItemViewModel> lsttmp = new List<OutwardItemViewModel>();

                for (int i = 0; i < tmp.Length; i++)
                {
                    for (int j = 0; j < tmpname.Length; j++)
                    {
                        for (int k = 0; k < tmpstatus.Length; k++)
                        {
                            for (int l = 0; l < tmpamount.Length; l++)
                            {
                        OutwardItemViewModel credit = new OutwardItemViewModel();
                                if (((i == j) && (i==l)) && (k == j))
                        {
                            credit.creditacc = tmp[i];
                            credit.creditaccname = tmpname[i];
                                    credit.creditpoststatus = tmpstatus[i];
                                    credit.creditchequeamount = (tmpamount[i]);
                                    credit.count = i + 1;
                            lsttmp.Add(credit); // store to List
                        }                        
                            }
                        }                      
                    }
                ...
                //List<OutwardItemViewModel> lsttmpname = new List<OutwardItemViewModel>();

                //for (int j = 0; j < tmpname.Length; j++)
                //{
                //    OutwardItemViewModel creditname = new OutwardItemViewModel();
                //    creditname.creditaccname = tmpname[j];
                //    creditname.count = tmpname.Length;
                //    lsttmpname.Add(creditname); // store to List
                //}

                ViewBag.lstCreditAcc = lsttmp;
                //ViewBag.lstCreditAccName = lsttmpname;

                return View(chequeVerificationPageHtml);
            }


            //public virtual async Task<ActionResult> ChequeAmountQueue(FormCollection collection)
            //{
            ...
            public virtual async Task<ActionResult> ChequeHistory(FormCollection collection)
            {
                ...
                else
                {
                    ...
                    ViewBag.SearchResult = await pageConfigDao.getResultListFromDatabaseViewAsync(new PageSqlConfig(TaskIdsOCS.AuditLogOCS.INDEX, "View_AudittrailOCS"
                      , "fldcreatetimestamp desc", "fldBankCode=@fldBankCode and fldTransNo = @fldTransNo", new[] { new SqlParameter("@fldBankCode", CurrentUser.Account.BankCode),
