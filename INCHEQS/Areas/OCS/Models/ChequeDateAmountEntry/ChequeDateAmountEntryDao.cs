...
namespace INCHEQS.Areas.OCS.Models.ChequeDateAmountEntry
{
    public class ChequeDateAmountEntryDao : IChequeDateAmountEntryDao
    {
        ...
        public ChequeDateAmountEntryDao(OCSDbContext dbContext, ISequenceDao sequenceDao, ICommonOutwardItemDao commonOutwardItemDao, IAuditTrailDao auditTrailDao, IClearingDao clearingDao)
        {
        ...
        public void Confirm(FormCollection col, AccountModel currentUser, string taskId)
        {
            try
            {
                ...
                //if (!col.AllKeys.Contains("chkWaive"))
                //{
                //    IsChargesWaive = "N";
                //}
                //else
                ...
                if (col["new_fldStateCode"] != col["current_fldLocation"])
                {
                    sqlUpdateInfo.Add("fldLocation", col["new_fldStateCode"]);
                    sqlChequeHistory.Add("fldOriLocation", col["current_fldLocation"]);
                    sqlChequeHistory.Add("fldLocation", col["new_fldStateCode"]);

                    auditLogBefore += " [Location] : " + col["current_fldLocation"];
                    auditLogAfter += " [Location] : " + col["new_fldStateCode"];
                    historyRemarks += "| Location changed from " + col["current_fldLocation"] + " to " + col["new_fldStateCode"] + ".";
                    changesCounter += 1;
                }
                ...
                sqlUpdateInfo.Add("fldreasoncode", DBNull.Value);
                sqlChequeHistory.Add("fldreasoncode", DBNull.Value);
                //sqlUpdateInfo.Add("fldItemType", "V");
                sqlUpdateInfo.Add("fldChequeIssueDate", col["txtChequeDate"]);
                sqlUpdateInfo1.Add("fldremark", col["textAreaRemarks"]);
                //sqlUpdateInfo.Add("fldWaiveCharges", IsChargesWaive);
               ...
               // auditTrailDao.Log("Cheque Amount Entry - Edit Item Info - Item Id : " + col["current_flditemid"] + " After Update=> " + auditLogAfter, currentUser);
                //outwardRemarks = "Cheque Amount Entry - Item has been Confirmed with following Information Item Id : " + col["current_flditemid"] + " Before Update=> " + auditLogBefore + " After Update=> " + auditLogAfter;
            ...
            if (CheckConfirm == "A")
            {
                ...
                else
                {
                    ...
                    if (col["new_fldserial"] != null)
                    {
                        ...
                        //}
                        else if (col["new_fldSerial"].ToString().Trim().Equals("000000"))
                        {
                            err.Add("Invalid Cheque number. Please enter correct Cheque number");
                        }
                    ...
                    {
                        if (col["new_fldissueraccno"].ToString().Equals("00000000000000000000") && col["new_fldtype"].ToString().Trim() == "1")
                        {
                    ...
                    if (col["new_fldbankCode"] != null)
                    {
                        ...
                        else if (col["new_fldbankCode"] == currentUser.BankCode)
                        {
                    ...
                    if (col["txtChequeAmount"] != "0.00" && col["txtChequeAmount"] != null && col["txtChequeAmount"] != "")
                    {
                        //if (Regex.IsMatch(col["txtchequedate"].ToString().Trim(), @"(((0|1)[0-9]|2[0-9]|3[0-1])\-(0[1-9]|1[0-2])\-(\d\d))$"))
                        if (ValidateDate(col["txtchequedate"].ToString().Trim()) == true)
                    {
                        string cdate = DateTime.Now.ToString(@"dd\-MM\-yy");
                        string chequedate = col["txtchequedate"].ToString();

                        DateTime chqDate = (DateTime.ParseExact(chequedate, @"dd\-MM\-yy", CultureInfo.InvariantCulture));
                        DateTime currentdate = (DateTime.ParseExact(cdate, @"dd\-MM\-yy", CultureInfo.InvariantCulture));

                        DateTime staledate = Convert.ToDateTime(currentdate.AddMonths(-6));
                        if (chqDate > currentdate)
                        {
                            err.Add("Invalid cheque. Post Dated Cheque.");
                        }
                        else if (chqDate <= staledate)
                        {
                            err.Add("Invalid cheque. Stale Dated Cheque.");
                        }
                    }
                    else
                    {
                            err.Add("Invalid cheque date {" + col["txtchequedate"].ToString().Trim() + "}. Please enter correct date format.");
                        } 
                    }
        ...
        public bool ValidateDate(string date)
        {
            ...
            if (date.Length == 8)
            {
                ...
                if (Convert.ToInt32(array[1]) > 12 || Convert.ToInt32(array[1]) < 1)
                {
                    Result = false;
                }
                else if (array[2].Length < 2 || array[2].Length > 2 || array[0].Length < 2 || array[0].Length > 2) 
                {
                    Result = false;
                }
                else
                {
                    ...
                    {
                        int numofDays = DateTime.DaysInMonth(Convert.ToInt32(Year.Trim()), Convert.ToInt32(array[1]));
                        if (numofDays < Convert.ToInt32(array[0]))
                        {
                            Result = false;
                        }
                    }
