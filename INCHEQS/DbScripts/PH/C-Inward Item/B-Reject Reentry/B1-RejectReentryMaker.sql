...
--create view
--------------------------------------------------------------
DROP VIEW [dbo].[View_RejectReentryMaker]
GO
CREATE VIEW [dbo].[View_RejectReentryMaker] --Maker
AS
Select * FROM View_AppInwardItem where 
	(fldRRstatus IS NULL) AND (fldRejectStatus1 IN (02,04,05)) OR
	(fldRRstatus IS NULL) AND (fldRejectStatus7 IS NOT NULL) OR
	(fldRRstatus IS NULL) AND (fldAccountNumber IN ('000000000000', '0000000000', '')) OR
	(fldRRstatus IS NULL) AND (fldChequeSerialNo IN ('0000000000', '')) OR
	(fldRRstatus = '1') OR
	(fldNonConfirmStatus = 'L')
GO
--------------------------------------------------------------
--Declaration
--------------------------------------------------------------
...
DECLARE @menuTitle varchar(100) = 'Reject Reentry Maker';
DECLARE @taskDesc varchar(100) = 'Inward Clearing - Reject Reentry Maker';

DECLARE @bank varchar(100) = '009';
DECLARE @view varchar(100) = 'View_RejectReentryMaker';
...
DECLARE @taskId varchar(100) = '309100';
DECLARE @lock varchar(100) = '309101';
DECLARE @action varchar(100) = '309102';
DECLARE @confirm varchar(100) = '309103';

--------------------------------------------------------------
--Add to tblQueueConfig
--------------------------------------------------------------
DELETE FROM [tblQueueConfig] WHERE [fldTaskId] = @taskId
INSERT [dbo].[tblQueueConfig] ([fldId], [fldTaskId], [fldQueueName], [fldQueueDesc], [fldViewName], [fldOrderBy], [fldUserParamCondition], [fldAllowedAction], [fldTaskRole], [fldLockCondition], [fldEnable], [fldEntityCode], [fldBankCode]) VALUES (4, @taskId, @menuTitle, @taskDesc, @view, @queueOrderBy, @userParam, @taskId+N','+@lock+N','+@action+N','+@confirm, @taskRole, @lockCondition, N'Y', @bank , @bank)
