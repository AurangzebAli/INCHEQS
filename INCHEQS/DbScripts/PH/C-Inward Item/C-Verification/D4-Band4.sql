...
--create view
--------------------------------------------------------------
DROP VIEW [dbo].[View_Band4]
GO
CREATE VIEW [dbo].[View_Band4]
AS
SELECT * FROM dbo.View_Verification
WHERE 
fldInwardItemId NOT IN (SELECT fldInwardItemId FROM View_RejectReentryMaker) AND 
(fldAmount >= (SELECT fldBandLBound FROM dbo.tblTaskBandLimit WHERE (fldTaskId = '306340'))) AND (fldAmount <= (SELECT fldBandUBound FROM dbo.tblTaskBandLimit AS tblTaskBandLimit_1 WHERE (fldTaskId = '306340')))
GO
...
DECLARE @menuTitle varchar(100) = 'Band 4';
DECLARE @taskDesc varchar(100) = 'Inward Clearing - Band 4';
DECLARE @mainMenuOrder int = 1;
DECLARE @subMenuOrder int = 11;

DECLARE @bank varchar(100) = '009';
DECLARE @view varchar(100) = 'View_Band4';
...
DECLARE @taskId varchar(100) = '306340';
DECLARE @lock varchar(100) = '306341';
DECLARE @action varchar(100) = '306342';
DECLARE @approve varchar(100) = '306343';
DECLARE @return varchar(100) = '306344';
DECLARE @route varchar(100) = '306345';
DECLARE @pullOut varchar(100) = '306346';
DECLARE @repair varchar(100) = '306347';
DECLARE @printAll varchar(100) = '306348';
DECLARE @printSum varchar(100) = '306349';

DECLARE @lowBound float = 1000000.00;
DECLARE @UpBound float = 9999999.99;
DECLARE @volumePercentage float = 0;

--------------------------------------------------------------
--Task Band Limit
--------------------------------------------------------------
DELETE FROM [tblTaskBandLimit] WHERE [fldTaskId] = @taskId
INSERT [dbo].[tblTaskBandLimit] ([fldTaskId], [fldOnUsId], [fldBandLBound], [fldBandUBound], [fldVolumnPercentage], [fldIsLastBand], [fldActive], [fldEntityCode], [fldBankCode]) VALUES (@taskId, @parentTaskId, @lowBound, @UpBound, @volumePercentage, N'N', N'Y', @bank, @bank)
