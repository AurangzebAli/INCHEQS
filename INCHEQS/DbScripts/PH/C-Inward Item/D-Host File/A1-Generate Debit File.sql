...
--create view
--------------------------------------------------------------
DROP VIEW [dbo].[View_GenerateDebitFile]
GO
CREATE VIEW [dbo].[View_GenerateDebitFile]
AS
SELECT *
FROM tblFileManager where fldTaskId = '309120'
GO
...
DECLARE @menuTitle varchar(100) = 'Generate Debit File';
DECLARE @taskDesc varchar(100) = 'Inward Clearing - Generate Debit File';
DECLARE @mainMenuOrder int = 1;
DECLARE @subMenuOrder int = 19;

DECLARE @bank varchar(100) = '009';
DECLARE @view varchar(100) = 'View_GenerateDebitFile';

DECLARE @systemProfileCode varchar(100) = '';
DECLARE @fileExt varchar(100) = '';
DECLARE @processName varchar(100) = 'ICSGenDebitFile';
DECLARE @posPayType varchar(100) = '';
DECLARE @FTPFolder varchar(100) = 'HostFile/';
DECLARE @seqTableName varchar(100) = 'GenDebitFile';
DECLARE @taskRole varchar(100) = '';

DECLARE @parentTaskId varchar(100) = '309000';
DECLARE @taskId varchar(100) = '309120';
...
INSERT [dbo].[tblSearchPageConfig] ([TaskId], [DatabaseViewId], [FieldId], [FieldIdSqlCondition], [FieldIdOrderBy], [FieldType], [FieldLabel], [FieldDefaultValue], [ValueTextQueryForOption], [PageTitle], [Length], [IsResultParam], [IsFilter], [IsResult], [IsReadOnly], [IsEnabled], [Ordering], [EntityCode], [BankCode]) VALUES (@taskId, @view, N'fldFilePath', NULL, NULL, N'', N'File Path', NULL, N'', @taskDesc, 10, N'Y', N'N', N'N', N'N', N'Y', 1, @bank, @bank)
