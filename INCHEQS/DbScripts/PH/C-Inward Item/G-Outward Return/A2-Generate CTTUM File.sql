...
--create view
--------------------------------------------------------------
DROP VIEW [dbo].[View_GenerateCTTUMFile]
GO
CREATE VIEW [dbo].[View_GenerateCTTUMFile]
AS
SELECT *
FROM tblFileManager where fldTaskId = '309210'
GO
...
DECLARE @menuTitle varchar(100) = 'Generate CTTUM File';
DECLARE @taskDesc varchar(100) = 'Inward Clearing - Generate CTTUM File';
DECLARE @mainMenuOrder int = 1;
DECLARE @subMenuOrder int = 26;

DECLARE @bank varchar(100) = '009';
DECLARE @view varchar(100) = 'View_GenerateCTTUMFile';

DECLARE @systemProfileCode varchar(100) = '';
DECLARE @fileExt varchar(100) = '';
DECLARE @processName varchar(100) = 'ICSGenerateHostFile_CTTUM';
DECLARE @posPayType varchar(100) = '';
DECLARE @FTPFolder varchar(100) = 'OutwardReturn/';
DECLARE @seqTableName varchar(100) = 'GenCTTUM';
DECLARE @taskRole varchar(100) = '';

DECLARE @parentTaskId varchar(100) = '309000';
DECLARE @taskId varchar(100) = '309210';
...
INSERT [dbo].[tblSearchPageConfig] ([TaskId], [DatabaseViewId], [FieldId], [FieldIdSqlCondition], [FieldIdOrderBy], [FieldType], [FieldLabel], [FieldDefaultValue], [ValueTextQueryForOption], [PageTitle], [Length], [IsResultParam], [IsFilter], [IsResult], [IsReadOnly], [IsEnabled], [Ordering], [EntityCode], [BankCode]) VALUES (@taskId, @view, N'fldHREJResponseMessage', NULL, NULL, N'', N'Message From Host', NULL, N'', @taskDesc, 10, N'Y', N'N', N'Y', N'N', N'Y', 1, @bank, @bank)

INSERT [dbo].[tblSearchPageConfig] ([TaskId], [DatabaseViewId], [FieldId], [FieldIdSqlCondition], [FieldIdOrderBy], [FieldType], [FieldLabel], [FieldDefaultValue], [ValueTextQueryForOption], [PageTitle], [Length], [IsResultParam], [IsFilter], [IsResult], [IsReadOnly], [IsEnabled], [Ordering], [EntityCode], [BankCode]) VALUES (@taskId, @view, N'fldFilePath', NULL, NULL, N'', N'Source', NULL, N'', @taskDesc, 10, N'Y', N'N', N'Y', N'N', N'Y', 1, @bank, @bank)
