...
--create view
--------------------------------------------------------------
DROP VIEW [dbo].[View_AppPendingDataChecker]
GO
CREATE VIEW [dbo].[View_AppPendingDataChecker]
AS
SELECT        *
FROM            dbo.View_AppPendingData
WHERE        fldApprovalStatusPending = 'H' OR fldApprovalStatusPending = 'J'
GO
--------------------------------------------------------------
--Declaration
--------------------------------------------------------------
...
DECLARE @menuTitle varchar(100) = 'Pending Data Checker';
DECLARE @taskDesc varchar(100) = 'Branch - Pending Data Checker';
DECLARE @mainMenuOrder int = 2;
DECLARE @subMenuOrder int = 2;

DECLARE @bank varchar(100) = '009';
DECLARE @view varchar(100) = 'View_AppPendingDataChecker';
...
DECLARE @taskId varchar(100) = '308120';
DECLARE @lock varchar(100) = '308121';
DECLARE @action varchar(100) = '308122';
DECLARE @approve varchar(100) = '308123';
DECLARE @referBack varchar(100) = '308124';
DECLARE @printAll varchar(100) = '308125';
DECLARE @printSum varchar(100) = '308126';
--------------------------------------------------------------
--Add to tblQueueConfig
--------------------------------------------------------------
DELETE FROM [tblQueueConfig] WHERE [fldTaskId] = @taskId
INSERT [dbo].[tblQueueConfig] ([fldId], [fldTaskId], [fldQueueName], [fldQueueDesc], [fldViewName], [fldOrderBy], [fldUserParamCondition], [fldAllowedAction], [fldTaskRole], [fldLockCondition], [fldEnable], [fldEntityCode], [fldBankCode]) VALUES (14, @taskId, N'Submission', @taskDesc, @view, @queueOrderBy, @userParam, @taskId+N','+@lock+N','+@action+N','+@approve+N','+@referBack+N','+@printAll+N','+@printSum , @taskRole, @lockCondition, N'Y', @bank, @bank)
...
DELETE FROM [tblTaskMaster] WHERE [fldTaskId] = @referBack
INSERT [dbo].[tblTaskMaster] ([fldBranchIndicator], [fldTaskId], [fldParentTaskID], [fldSubMenuTaskID], [fldMenuType], [fldMenuDesc], [fldButtonType], [fldSeqNo], [fldSearchFlag], [fldEditPage], [fldTaskType], [fldMainMenu], [fldSubMenu], [fldMainSeq], [fldSubSeq], [fldMenuTitle], [fldPageTitle], [fldTaskDesc], [fldSearch], [fldAllowNew], [fldImgFileName], [fldEditFile], [fldWidth], [fldHeight], [fldListImage], [fldPageImage], [fldBrowsePage], [fldRecordStatus], [fldMvcUrl]) 
VALUES (N'H', @referBack, @parentTaskId, NULL, N'S', N'', N' ', NULL, N' ', N'', N'', N'', N'', 1, 2, N'', N'', @taskDesc+N' (Branch ReferBack)', N'N', N'N', N'', N'', 0, 0, N'', N'', N'', 1, N'BranchReferBack')
