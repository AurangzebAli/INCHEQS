...
--create view
--------------------------------------------------------------
DROP VIEW [dbo].[View_ReviewMineBranch]
GO
CREATE VIEW [dbo].[View_ReviewMineBranch]
AS
select distinct a.*,
	(select fldUserAbb from tblUserMaster where fldUserId = b.fldCreateUserId) AS fldInvolvedUserName ,
	(select fldUserId from tblUserMaster where fldUserId = b.fldCreateUserId) AS fldInvolvedUserId 
FROM View_AppPendingData a left outer join
	View_ChequeHistory b on a.fldInwardItemId = b.fldInwardItemId
GO
...
DECLARE @menuTitle varchar(100) = 'Review Mine';
DECLARE @taskDesc varchar(100) = 'Branch - Review Mine';
DECLARE @mainMenuOrder int = 2;
DECLARE @subMenuOrder int = 3;

DECLARE @bank varchar(100) = '009';
DECLARE @view varchar(100) = 'View_ReviewMineBranch';
DECLARE @taskRole varchar(100) = '';
DECLARE @queueOrderBy varchar(100) = 'fldAccountDesc DESC, fldAmount DESC';
DECLARE @userParam varchar(100) = 'fldInvolvedUserId=@currentUserId AND fldBranchCode IN (@currentUserBranchCodes)';
DECLARE @lockCondition varchar(100) = '';

DECLARE @parentTaskId varchar(100) = '308000';
DECLARE @taskId varchar(100) = '308210';
DECLARE @printAll varchar(100) = '308211';
--------------------------------------------------------------
...
INSERT INTO [dbo].[tblReportPageConfig] ([ReportId], [TaskId], [DatabaseViewId], [ReportPath], [ReportTitle], [DataSetName], [exportFileName],[PrintReportParam], [IsEnabled], [Ordering]) VALUES (N'3', @taskId, @view, N'~//Reports/ICS/ExtractedInwardData.rdlc', @taskDesc, N'ExtractedInwardDataSet', N'ExtractedInwardRpt', N'sum', N'Y', 1)

--------------------------------------------------------------
--Add to tblQueueConfig
--------------------------------------------------------------
DELETE FROM [tblQueueConfig] WHERE [fldTaskId] = @taskId
INSERT [dbo].[tblQueueConfig] ([fldId], [fldTaskId], [fldQueueName], [fldQueueDesc], [fldViewName], [fldOrderBy], [fldUserParamCondition], [fldAllowedAction], [fldTaskRole], [fldLockCondition], [fldEnable], [fldEntityCode], [fldBankCode]) VALUES (1, @taskId, @menuTitle, @taskDesc, @view, @queueOrderBy,@userParam ,@printAll, @taskRole ,@lockCondition , N'Y', @bank , @bank)
...
INSERT [dbo].[tblSearchPageConfig] ([TaskId], [DatabaseViewId], [FieldId], [FieldIdSqlCondition], [FieldIdOrderBy], [FieldType], [FieldLabel], [FieldDefaultValue], [ValueTextQueryForOption], [PageTitle], [Length], [IsResultParam], [IsFilter], [IsResult], [IsReadOnly], [IsEnabled], [Ordering], [EntityCode], [BankCode]) VALUES (@taskId, @view, N'fldMaxAmount', N'fldAmount <= @@param', NULL, N'NumberBox', N'Max Amount', NULL, N'', @taskDesc, 999, N'N', N'Y', N'N', N'N', N'Y', 2, @bank, @bank)
