...
--create view
--------------------------------------------------------------
Drop View [dbo].[View_FreeItemReport]
Go
CREATE VIEW [dbo].[View_FreeItemReport]
AS
SELECT (ci.fldIssueStateCode + ci.fldIssueBankCode + ci.fldIssueBranchCode + ci.fldIssueDigit) AS fldBRSTN, ci.fldClearDate,
...
DECLARE @menuTitle varchar(100) = 'Generate Free Item Report';
DECLARE @taskDesc varchar(100) = 'Report - Generate Free Item Report';
DECLARE @mainMenuOrder int = 7;
DECLARE @subMenuOrder int = 63;

DECLARE @bank varchar(100) = '009';
DECLARE @view varchar(100) = 'View_FreeItemReport';
DECLARE @reportPath varchar(100) = '~//Reports/ICS/FreeItemReport.rdlc';
DECLARE @reportTitle varchar(100) = 'FreeItemReport';
DECLARE @fileName varchar(100) = 'FreeItemRpt';
DECLARE @reportParam varchar(100) = '';
DECLARE @orientation varchar(100) = 'Landscape';

DECLARE @parentTaskId varchar(100) = '304000';
DECLARE @taskId varchar(100) = '304530';
--------------------------------------------------------------
--Update Taskmaster
--------------------------------------------------------------
