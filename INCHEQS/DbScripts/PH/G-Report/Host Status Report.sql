...
--create view
--------------------------------------------------------------
Drop View [dbo].[View_HostStatusReport]
Go
CREATE VIEW [dbo].[View_HostStatusReport]
AS
select  fldClearDate,fldAccountNumber,fldChequeSerialNo,fldRejectCode, fldRejectDesc,fldAmount,fldBranchCode from View_AppInwardItem where fldRejectCode not in('','00')
GO
--------------------------------------------------------------
--Declaration
--------------------------------------------------------------
...
DECLARE @menuTitle varchar(100) = 'Host Status Report';
DECLARE @taskDesc varchar(100) = 'Report - Host Status Report';
DECLARE @mainMenuOrder int = 7;
DECLARE @subMenuOrder int = 63;

DECLARE @bank varchar(100) = '009';
DECLARE @view varchar(100) = 'View_HostStatusReport';
DECLARE @reportPath varchar(100) = '~//Reports/ICS/HostStatusReport.rdlc';
DECLARE @reportTitle varchar(100) = 'HostStatusReport';
DECLARE @fileName varchar(100) = 'HostStatusRpt';
DECLARE @reportParam varchar(100) = '';
DECLARE @orientation varchar(100) = 'Landscape';

DECLARE @parentTaskId varchar(100) = '304000';
DECLARE @taskId varchar(100) = '304550';
--------------------------------------------------------------
--Update Taskmaster
--------------------------------------------------------------
