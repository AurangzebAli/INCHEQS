...
--create view
--------------------------------------------------------------
Drop View [dbo].[View_RejectedInwardReport]
Go
CREATE VIEW [dbo].[View_RejectedInwardReport]
AS
select  fldClearDate,fldBranchCode,fldRejectCode,fldRejectDesc,fldUserAbb,fldChequeSerialNo,fldAccountNumber,fldAmount from View_AppInwardItem --where??
GO
...
DECLARE @menuTitle varchar(100) = 'Rejected Inward Report';
DECLARE @taskDesc varchar(100) = 'Report - Rejected Inward Report';
DECLARE @mainMenuOrder int = 7;
DECLARE @subMenuOrder int = 63;

DECLARE @bank varchar(100) = '009';
DECLARE @view varchar(100) = 'View_RejectedInwardReport';
DECLARE @reportPath varchar(100) = '~//Reports/ICS/RejectedInwardReport.rdlc';
DECLARE @reportTitle varchar(100) = 'RejectedInwardClearingReport';
DECLARE @fileName varchar(100) = 'RejectedInwardClearingRpt';
DECLARE @reportParam varchar(100) = '';
DECLARE @orientation varchar(100) = 'Landscape';

DECLARE @parentTaskId varchar(100) = '304000';
DECLARE @taskId varchar(100) = '304580';
--------------------------------------------------------------
--Update Taskmaster
--------------------------------------------------------------
