...
DashboardFunctions = {
    ...
    Table: function (selector, data) {
        var $holder = $(selector);
        $holder.html(_.template(OCSDashboardTempates.TableTemplate)({ data: data }));
    },

    QueueTableTemplate: function (selector, data) {
        var $holder = $(selector);
        $holder.html(_.template(OCSDashboardTempates.QueueTableTemplate)({ data: data }));

        //Draw total for each column after success load
        $("#progressStatusWidget table tbody tr").first().children().each(function () {
    ...
    PaginatedTable: function (selector, data) {
        var $holder = $(selector);
        $holder.html(_.template(OCSDashboardTempates.PaginatedTableTemplate)({ data: data }));
